...
import helpers.util.ResultSetMapper;
import pojo.lombok.javafaker.customer.Customer;
import pojo.lombok.javafaker.customer.model.CustomerPojo;
...
public class CustomerDaoImpl implements CustomerDao<Customer>, Queries {
    ...
    @Override
    public CustomerPojo getByID(int id) {
        List<CustomerPojo> customers;
        ResultSetMapper<CustomerPojo> resultSetMapper = new ResultSetMapper<>();
        try {
            this.preparedStatement = this.conn.prepareStatement(Queries.selectCustomerPojoQuery);

            preparedStatement.setInt(1, id);
            ResultSet rs = preparedStatement.executeQuery();
            customers = resultSetMapper.mapResultSetToObject(rs, CustomerPojo.class);
            if (customers.size() > 0) {
                return customers.get(0);
            } else {
                System.out.printf("There is no record with id = {d}", id);
                return null;
            }
    ...
    @Override
    public List<CustomerPojo> getByIDs(List<Integer> ids) {
